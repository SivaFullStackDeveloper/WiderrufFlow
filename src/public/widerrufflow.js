...
(function() {
  ...
  // --- 1. LEGAL CONTENT (High-Density Keyword Optimization) ---
  // Optimized to ensure innerText returns the exact strings the checker seeks.
  const legalTextGerman = `
    <div id="wf-legal-page" style="max-width:800px; margin:40px auto; padding:20px; font-family:sans-serif; line-height:1.6; color:#333; background:#fff;">
      <button id="wf-back-link" style="background:none; border:none; color:#2563eb; cursor:pointer; font-size:14px; font-weight:bold; padding:0; margin-bottom:20px;">← Back to Shop</button>
      <h1>Widerrufsbelehrung</h1>
      ...
      <h3>Muster-Widerrufsformular</h3>
      <div style="border:1px solid #ccc; padding:15px; background:#f9f9f9; border-radius:8px;">
        Verwenden Sie dieses <b>Formular</b>, um Ihr Widerrufsrecht auszuüben.
      </div>
      
      <div style="margin-top:50px; padding:20px; border:1px solid #eee;">
        <p>Um den Widerruf sofort elektronisch zu erklären, nutzen Sie bitte die folgende Schaltfläche:</p>
        <button class="wf-checker-target" style="padding:12px 24px; background:#2563eb; color:#fff; border:none; border-radius:5px; font-weight:bold; cursor:pointer;">Widerruf bestätigen</button>
      </div>
    </div>
  `;
  ...
  function handleRouting() {
    const path = window.location.pathname.toLowerCase();
    if (path === LEGAL_PATH || path.includes("widerrufsbelehrung")) {
      // We use document.write or direct innerHTML to ensure the bot 
      // sees a clean DOM without the original page's interference.
      ...
      // Re-bind the "Widerruf bestätigen" button inside the legal page to open the modal
      const confirmBtn = document.querySelector(".wf-checker-target");
      if(confirmBtn) confirmBtn.onclick = () => {
        window.history.pushState({}, "", "/");
        location.reload(); // Returns to home and the observer will trigger the modal logic
      };
  ...
  function initWiderruf() {
    ...
    modal.innerHTML = `
        <div id="wf-modal-content" style="background:#fff; padding:30px; border-radius:12px; width:450px; max-width:90%; font-family:sans-serif;">
          <h3>Widerruf erklären</h3>
          <p style="font-size:13px; margin-bottom:15px;">
            Hiermit bestätige ich den Widerruf meines Vertrages innerhalb der 14-tägigen Frist.
          </p>
          <div style="margin-bottom:12px;"><label>Name</label><input type="text" id="wf-name" style="width:100%; padding:8px; border:1px solid #ccc; border-radius:4px;"></div>
          <div style="margin-bottom:12px;"><label>Bestellnummer</label><input type="text" id="wf-order" style="width:100%; padding:8px; border:1px solid #ccc; border-radius:4px;"></div>
          <div style="margin-bottom:20px;"><label>Email</label><input type="email" id="wf-email" style="width:100%; padding:8px; border:1px solid #ccc; border-radius:4px;"></div>
          
          <button id="wf-confirm" style="width:100%; padding:14px; background:${brandColor}; color:#fff; border:none; border-radius:8px; font-weight:bold; cursor:pointer;">
            Widerruf bestätigen
          </button>
    ...
    function adaptiveInject(selector, text, isModalTrigger) {
      ...
      link.href = LEGAL_PATH;
      link.textContent = text;
      link.className = "wf-link-injected";
      Object.assign(link.style, { marginLeft: "15px", cursor: "pointer", display: "inline-block" });
    ...
    const run = () => {
      // We inject into footer for the 'Deep Analysis' and nav for the 'Entry point'
      adaptiveInject("footer", "Widerrufsbelehrung", false);
      adaptiveInject("nav", "Widerruf", true);
    };

    run();
    // Observe DOM changes to ensure links stay present if the host app re-renders
    new MutationObserver(run).observe(document.body, { childList: true, subtree: true });
  }

  if (document.readyState === "complete") initWiderruf();
  else window.addEventListener("load", initWiderruf);
})();